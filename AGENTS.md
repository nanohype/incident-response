...
| `webhook-ingress.yaml`                                  | `alb`-class Ingress — TLS terminates on the ALB against the ACM certificate `ingress.tls.certificateArn` names (no in-cluster certificate, no TLS Secret). `POST /webhook` for Grafana OnCall + `/slack` prefix for the Slack Request URLs |
...
| `externalsecret.yaml`                                   | ESO syncs one entry per integration from `incident-response/<env>/` into one Secret consumed via `envFrom`, at the paths `scripts/seed-secrets.sh` writes (CI parity gate); the HMAC secret is not projected — its id is passed as env and the handler reads it directly for the VersionId-keyed cache refresh. No OTLP credential — the default export target is the unauthenticated in-cluster collector gateway receiver |
| `networkpolicy.yaml`                                    | Default-deny + ingress (the ALB's VPC range → webhook only) + egress (DNS + HTTPS)                                                                                           |
| `prometheusrule.yaml`                                   | SLO + reliability alerts. `enabled: false` by default — inert without a Prometheus Operator ruler, which the eks-gitops observability stack (OpenTelemetry Collector → Amazon Managed Prometheus) does not run |
