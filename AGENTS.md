...
| `externalsecret.yaml`                                   | ESO syncs one entry per integration from `incident-response/<env>/` into one Secret consumed via `envFrom`, at the paths `scripts/seed-secrets.sh` writes (CI parity gate); the HMAC secret is not projected — its id is passed as env and the handler reads it directly for the VersionId-keyed cache refresh. No OTLP credential — the default export target is the unauthenticated in-cluster Alloy receiver |
