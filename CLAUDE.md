...
> Internal service handle: `incident-response`. The npm package, the OTel `service.namespace` / `agents.platform`, the `/incident-response` slash commands + Slack app, and the `incident-response/<env>/*` secret prefixes all stay `incident-response` — they're coupled to the landing-zone `incident-response-platform` substrate component.

## What This Is

A protohype subsystem composing nanohype templates (`ts-service` worker-service variant, `infra-aws`, `agentic-loop`, `prompt-library`, `module-llm`) into a long-running Slack-socket-mode daemon. A webhook Deployment behind ingress-nginx ingests Grafana OnCall alerts; a processor Deployment runs the Slack socket-mode singleton.
...
- **chart/** — Helm chart for the k8s deployment. `Chart.yaml`, `values.yaml`, per-env deltas (`values-{staging,production}.yaml`), and templates under `chart/templates/`: `webhook-deployment.yaml` + `webhook-service.yaml` + `webhook-ingress.yaml` (the webhook Deployment running `src/bin/webhook-server.ts`, served publicly via ingress-nginx for the Grafana OnCall HMAC POSTs), `processor-deployment.yaml` (Slack socket-mode singleton, `Recreate` strategy, 60s `terminationGracePeriodSeconds` for in-flight SQS drain), `serviceaccount.yaml` (shared SA named `incident-response`, bound to the landing-zone `incident-response-platform` IAM role by an EKS Pod Identity association — no role-arn annotation), `networkpolicy.yaml` (ingress-nginx → webhook only; egress: DNS + HTTPS), `externalsecret.yaml` (pulls incident-response/<env>/grafana-oncall-hmac + app-secrets + grafana-cloud into one Secret consumed via envFrom; HMAC secret is also passed as `GRAFANA_ONCALL_HMAC_SECRET_ID` env for the handler's VersionId-keyed cache refresh), `prometheusrule.yaml` (three SLO + reliability rules), `grafana-dashboard.yaml` (ConfigMap labeled `grafana_dashboard: "1"`, auto-imported by the kube-prometheus-stack Grafana sidecar from `chart/dashboards/incident-response.json`). Observability is cluster-level via eks-gitops: Pino JSON → cluster log forwarder → Grafana Cloud Loki; OTLP → `grafana-agent.monitoring.svc.cluster.local:4318` → Grafana Cloud Tempo + Mimir. See `chart/README.md` for the full template-by-template description.
- **platform.yaml** — Platform CR (`platform.nanohype.dev/v1alpha1`) plus a co-declared BudgetPolicy (`governance.nanohype.dev/v1alpha1`) declaring incident-response as a tenant of the `protohype` team on the `eks-agent-platform` operator. Operator reconciles Namespace `tenants-protohype`, ResourceQuota (4 CPU / 8Gi memory), LimitRange, default-deny NetworkPolicy, ArgoCD AppProject, IAM role with placeholder ARNs for dynamodb-rw, sqs-consumer, bedrock-invoke, scheduler-rw, secrets-read, cloudwatch-putmetric, KMS grants on `cmk-data`, S3 bucket policy on `incident-response-artifacts`.
...
- **src/bin/webhook-server.ts** — Thin `node:http` wrapper around `src/handlers/webhook-ingress.ts`'s `APIGatewayProxyHandlerV2`. Constructs an `APIGatewayProxyEventV2`-shaped object from the inbound request, calls the handler, incident-responses the response back. No new runtime dependencies (node:http is built-in; `@types/aws-lambda` is a devDependency that supplies the handler's event/response types). This is the entrypoint the webhook Deployment runs; it mounts the handler on POST plus `/health` for k8s probes.
...
npm run sync:vendored               # re-copy vendored chart + runtime modules from ../nanohype (or $NANOHYPE_DIR)
