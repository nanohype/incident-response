...
- **src/utils/metrics.ts** — `MetricsEmitter` over the OTel Metrics API, on top of the vendored runtime metrics module (namespace-qualified `incident_response.*` series, per-name instrument caching, no-op without a provider). Fire-and-forget; the SDK buffers and batches, so emission never blocks a caller. `count(name, value, dims)` adds an arbitrary amount where `increment` only ever adds 1 — token counts are quantities, not events — and drops non-finite or negative values, since a monotonic counter cannot hold them and one NaN poisons every sum over the series.
...
- **src/vendor/runtime/** — vendored `@nanohype/runtime` modules (`circuit-breaker.ts`, `resilience.ts`, `pii.ts`, `workos-directory.ts`), byte-identical to `nanohype/library/runtime/src/*` — the same consumption model as the vendored `chart/charts/tenant-chart-base`. `scripts/sync-vendored.mjs` (itself vendored from `library/scripts/`, driven by `scripts/vendored.json`) re-vendors every surface — runtime modules, `biome.base.json`, the chart base — from a nanohype checkout (`NANOHYPE_DIR`, default `../nanohype`) read through git objects at the commit pinned in `scripts/vendored.json`. `--ref=<sha>` re-vendors and moves the pin together, so the recorded commit and the bytes on disk cannot describe different things; CI's `--check` mode compares against that pin, so a merge in nanohype never turns this repo's required check red, and `--freshness` asks whether the pin has fallen behind on a weekly schedule instead. Behavior changes land upstream first, with their tests — never edit the copies. Vendored files are excluded from app lint/format/coverage (enforced upstream); `tsc` still typechecks and compiles them.
...
- **chart/** — Helm chart for the k8s deployment. `Chart.yaml`, `values.yaml`, per-env deltas (`values-{staging,production}.yaml`), and templates under `chart/templates/`: `webhook-deployment.yaml` + `webhook-service.yaml` + `webhook-ingress.yaml` (the webhook Deployment running `src/bin/webhook-server.ts`, served publicly via the cluster's ingress controller for the Grafana OnCall HMAC POSTs and the Slack signed-HTTP Request URLs on the `/slack` path prefix), `processor-deployment.yaml` (single-writer singleton — SQS consumer + assembler + in-process MCP server on the `mcp` container port; `Recreate` strategy, 60s `terminationGracePeriodSeconds` for in-flight SQS drain), `mcp-service.yaml` (ClusterIP in front of the processor's MCP port — the mcp-tunnel target), `serviceaccount.yaml` (the operator-owned `tenant-runtime` SA, bound to the operator-reconciled `<env>-incident-response-tenant` IAM role by a Pod Identity association the operator creates for the `tenant-runtime` ServiceAccount — no role-arn annotation), `networkpolicy.yaml` (the VPC range the ALB's interfaces sit in → webhook; mcp-tunnel namespace → the processor MCP port; egress: DNS + HTTPS + OTLP), `externalsecret.yaml` (projects one entry per integration from incident-response/<env>/ into one Secret consumed via envFrom, each at the path `scripts/seed-secrets.sh` writes — a CI parity gate renders the chart and fails on any path the seeder does not create; the HMAC secret is deliberately not projected, its id goes out as `GRAFANA_ONCALL_HMAC_SECRET_ID` and the handler reads it through the pod's own grant for the VersionId-keyed cache refresh; no OTLP credential is projected either, since the export target is the unauthenticated in-cluster collector gateway receiver), `prometheusrule.yaml` (three SLO + reliability rules), `grafana-dashboard.yaml` (a `GrafanaDashboard` CR the grafana-operator reconciles, sourced from `chart/dashboards/incident-response.json`). Observability is cluster-level via eks-gitops, all of it through the OpenTelemetry Collector gateway in the `monitoring` namespace: Pino JSON on stderr → the collector's pod tail → the in-cluster Loki; OTLP on `telemetry.monitoring.svc.cluster.local:4318` → the in-cluster Tempo (traces) + SigV4 remote-write to Amazon Managed Prometheus (metrics). See `chart/README.md` for the full template-by-template description.
- **platform.yaml** — the cluster-scoped `Tenant` CR for the `reliability` team, plus the Platform CR (`platform.nanohype.dev/v1alpha1`) and its co-declared BudgetPolicy (`governance.nanohype.dev/v1alpha1`) declaring incident-response as a tenant of that team on the `eks-agent-platform` operator. The Platform + BudgetPolicy live in the team's control-plane namespace `tenants-reliability`; the operator provisions the workload namespace `tenants-incident-response` from the Platform name, along with ResourceQuota (4 CPU / 8Gi memory), LimitRange, default-deny NetworkPolicy, an ArgoCD AppProject named `incident-response`, and the per-Platform IAM role `<env>-incident-response-tenant` the chart's ServiceAccount is bound to. CI validates every document in this file against the CRD schemas vendored under `schemas/crd/` — see `scripts/validate-platform-manifests.mjs`. Those schemas are digest-verified against `schemas/crd/source.json` before use, and `scripts/sync-crd-schemas.mjs --check` compares them to `nanohype/eks-agent-platform` at the pinned commit. Whether that pin has fallen behind upstream is asked separately by `--freshness` on a weekly schedule, so the blocking gate stays a pure function of the commit under test.
...
npm run schemas:sync                # re-vendor schemas/crd/ from the pinned ref ($EKS_AGENT_PLATFORM_DIR, else GitHub)
npm run schemas:check               # blocking gate — digests, and bytes vs upstream at the pinned commit
