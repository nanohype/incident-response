...
The AWS substrate — DynamoDB tables, SQS + DLQ, the EventBridge Scheduler group, the S3 audit/artifacts bucket, and the `marshal_irsa` role — lives in the `marshal-platform` component in [`landing-zone`](https://github.com/nanohype/landing-zone). Apply it per env with OpenTofu/Terragrunt before deploying the tenant; its `tofu output` values feed the chart (see step 3). You do not provision AWS resources from this repo.
...
Every secret is operator-provisioned — the chart's ExternalSecret references them by name but does not create them. The processor pod won't start cleanly until the External Secrets Operator can resolve every `marshal/<env>/*` entry, so seed first.
...
Apply the `marshal-platform` component in `landing-zone` for this env (OpenTofu/Terragrunt), then apply the tenant boundary:

```bash
kubectl apply -f platform.yaml      # Platform CR + BudgetPolicy
kubectl -n tenants-protohype get platform marshal -w   # wait for Ready
```
...
Fill the per-env values from `tofu output` against the `marshal-platform` component — do **not** hardcode account IDs / ARNs into committed defaults:

```bash
# In landing-zone/live/<env>/aws/marshal-platform
...
```bash
kubectl -n tenants-protohype rollout status deploy/marshal-webhook
kubectl -n tenants-protohype rollout status deploy/marshal-processor

# ExternalSecret synced into a k8s Secret?
kubectl -n tenants-protohype get externalsecret marshal -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}'

# Webhook HMAC gate is live (unsigned request → 401):
kubectl -n tenants-protohype port-forward svc/marshal-webhook 3001:80 &
...
# Fill chart/values-production.yaml from the production marshal-platform tofu output (step 3),
...
Removing the tenant is the inverse of the deploy: remove the ApplicationSet entry (ArgoCD prunes the workloads), then `kubectl delete -f platform.yaml`. The DynamoDB tables, the S3 bucket, and the Secrets Manager entries are owned by `landing-zone` (`marshal-platform`), not by this repo — they survive a tenant teardown so a rebuild can reuse them. To fully remove an environment's data, destroy the `marshal-platform` component in `landing-zone` and delete the secret tree:
...
|---|---|---|
| ExternalSecret stuck `SecretSyncedError` | A `marshal/<env>/*` secret doesn't exist yet, or the IRSA role lacks `GetSecretValue` on its ARN | Run step 1 for that env; confirm the `marshal_irsa` role's secrets-read scope in `landing-zone` |
| Processor pod `CrashLoopBackOff` | Zod config fail on startup — one of the per-integration secrets is empty for this env | `kubectl logs deploy/marshal-processor -n tenants-protohype` and look for the missing key; reseed + `kubectl rollout restart` |
...
| Webhook returns 5xx on unsigned POST | Webhook pod crashed before the HMAC check | `kubectl logs deploy/marshal-webhook -n tenants-protohype`. Usually a missing `GRAFANA_ONCALL_HMAC_SECRET_ID` or a Secrets Manager permission regression |
| DLQ depth > 0 | An incident event failed 3 times | Inspect + drain via `aws sqs receive-message`; the PrometheusRule fires on the `marshal-{env}-incident-events` DLQ at ≥1 |
...
| Resolve fires but "Bedrock postmortem failed" in logs | `claude-sonnet-4-6` requires an inference profile for on-demand throughput | Switch `src/ai/marshal-ai.ts` model IDs to `us.anthropic.claude-*` profiles. See [`docs/troubleshooting.md`](troubleshooting.md) § "Bedrock errors" |
| Resolve fires but Linear issue doesn't appear | `teamId must be a UUID` — `linear/team-id` secret holds a team key | Reseed with the UUID from `{ teams { nodes { id key } } }`; `kubectl rollout restart deploy/marshal-processor` |
| Nudge schedule never fires (no `STATUS_REMINDER_SENT` after 15 min) | `Schedule group marshal-{env} does not exist` in processor logs | The group is owned by `landing-zone marshal-platform`; confirm it's applied. Details in [`docs/troubleshooting.md`](troubleshooting.md) § "EventBridge Scheduler errors" |
...
| Secret rotated but pods still use old value | Pods read the projected Secret at start | `kubectl rollout restart deploy/marshal-processor -n tenants-protohype` (and `deploy/marshal-webhook`) to pick up the new value |
