...
This app is CommonJS by design: `package.json` carries no `"type": "module"` and `tsconfig.json` emits `module: commonjs`. The org convention is ESM-everywhere, and an ESM migration is **deferred, not forgotten** — the runtime is Node 24, every dependency is CJS-compatible, and the 100%-branch-coverage invariant files (`audit.ts`, `statuspage-approval-gate.ts`) are the kind of code an in-flight runner/module migration most easily breaks. Keeping CommonJS removes the "looks like ESM but isn't" ambiguity, which is the real footgun. The same reasoning keeps the test runner on **Jest** (`jest.config.cjs` + `jest.config.integration.cjs`) rather than Vitest — the standard requires `npm test` to pass, it does, and the 100%-branch enforcement stays intact. Test files are type-checked separately (they're excluded from the build `tsc`), so test type drift still surfaces.
