...
### Single-writer singleton processor

The processor is single-writer: it owns stateful war-room assembly and the SQS consumer, and it hosts the in-process streamable-HTTP MCP server. Two processors briefly running would each poll SQS and dispatch the same incident event, doubling actions (a second Slack war room, duplicate audit writes). So `processor-deployment.yaml` runs `replicaCount: 1` with `strategy: Recreate` (not `RollingUpdate`): a deploy takes the old pod down before the new one comes up, accepting brief downtime over a double-dispatch window. `terminationGracePeriodSeconds: 60` gives the in-flight SQS messages time to drain on shutdown. The webhook Deployment is the opposite — stateless HTTP, 2+ replicas, rolling restarts — because it only verifies signatures (Grafana HMAC / Slack signing secret) and enqueues or dispatches. The SQS consumer is DLQ-safe: it does not `DeleteMessage` on a handler exception, so the 300s visibility timeout drives retry and then the DLQ.

### No Slack socket mode — signed HTTP + MCP

Slack interactions arrive as signature-verified HTTP, not over Bolt/socket mode. Slack posts slash commands and Block Kit actions to a Request URL; the webhook Deployment already spoke raw signed HTTP for Grafana, so the Slack slash-command and interactivity endpoints (`/slack/commands`, `/slack/interactivity`) live there too, verified with the Slack signing secret (`src/handlers/slack-signature.ts`). The read + draft pull surface is a streamable-HTTP MCP server on the processor (`src/mcp/`), reached by Claude surfaces over the outbound mcp-tunnel. The approval-gate boundary draws the line: the MCP tools read and *draft* (`draft_statuspage_update` → `PENDING_APPROVAL`), and a **human approves** the customer-facing publish with a deterministic, fully-attributed Slack button (`statuspage_approve` → `approveAndPublish(..., body.user.id)`). Approve/publish/resolve are never MCP tools — that is the whole point of the split.

### CommonJS — deliberate, not a mislabel

This app is CommonJS by design: `package.json` carries no `"type": "module"` and `tsconfig.json` emits `module: commonjs`. The org convention is ESM; this repo deliberately diverges. The runtime is Node 24 and every dependency is CJS-compatible, so the choice costs nothing at runtime — and it removes the "looks like ESM but isn't" ambiguity, which is the real footgun around the 100%-branch-coverage invariant files (`audit.ts`, `statuspage-approval-gate.ts`). The test runner is **Vitest** (`vitest.config.ts` + `vitest.config.integration.ts`) with the v8 coverage provider — the standard requires `npm test` to pass, it does, and the 100%-branch enforcement on `audit.ts` + `statuspage-approval-gate.ts` stays intact via per-file thresholds. Test files are type-checked separately (they're excluded from the build `tsc`), so test type drift still surfaces.
...
- **the AWS Load Balancer Controller** — backs `webhook-ingress.yaml`. It creates the `alb` IngressClass this chart requests and provisions an internet-facing ALB registering pod IPs directly (Cilium runs ENI IPAM with native routing, so pod IPs are VPC addresses). TLS terminates on the load balancer against an ACM certificate — set `ingress.tls.certificateArn`, or leave it empty and let the controller match one to the rule host. There is no in-cluster certificate and no TLS Secret
- **external-dns** — runs with `sources: [service, ingress]` and publishes the Route53 record for `ingress.host` from the Ingress itself, so the Grafana OnCall and Slack Request URLs resolve without a separate DNS step
- **observability stack** — the OpenTelemetry Collector gateway in the `monitoring` namespace, doing both jobs: its gateway receives OTLP on `:4318` and its agent tails pod stdout/stderr, then fans out to the in-cluster Tempo (traces), Amazon Managed Prometheus via SigV4 remote-write (metrics), and the in-cluster Loki (logs). The app emits OTLP and structured Pino JSON to stderr; there are no per-pod sidecars. The `grafana-dashboard.yaml` `GrafanaDashboard` CR (`chart/dashboards/incident-response.json`) loads into the org Grafana and queries the `incident-response`-namespaced telemetry. `prometheusrule.yaml` is off by default — eks-gitops ships the prometheus-operator CRDs but no operator to act on them.
