#!/usr/bin/env bash
#
# Fire a synthetic Grafana OnCall alert at IncidentResponse's webhook ingress.
#
...
#      (incident-response/${env}/grafana/oncall-webhook-hmac)
...
STACK_NAME="IncidentResponse${ENVIRONMENT^}"                       # IncidentResponseStaging / IncidentResponseProduction
...
HMAC_SECRET=$(aws secretsmanager get-secret-value --region "$REGION" \
  --secret-id "incident-response/${ENVIRONMENT}/grafana/oncall-webhook-hmac" \
  --query 'SecretString' --output text 2>/dev/null || true)
[[ -n "$HMAC_SECRET" ]] || die "could not read incident-response/${ENVIRONMENT}/grafana/oncall-webhook-hmac — has it been seeded?"
...
case "$STATUS" in
  200)
    ...
    log "  • Logs:  aws logs tail /incident-response/$ENVIRONMENT/processor --region $REGION --follow"
