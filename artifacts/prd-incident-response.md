# Product Requirements Document — IncidentResponse
**Version:** 1.0  
...
**IncidentResponse is a ceremonial assistant for incident commanders.** It handles the ceremony so the IC can focus on resolving the incident.
...
| IC pulse rating "IncidentResponse helped me think clearly" | n/a | ≥4/5 over rolling 10 incidents | Slack ephemeral emoji-rating at incident resolution |
...
| IncidentResponse-drafted status messages published without IC approval | n/a | **0** (100% gate) | Audit log query: `published_messages` WHERE `approved = false` |
...
- Will rate IncidentResponse 1-5 at resolution; low scores are a direct signal of bad behavior
...
**Trigger:** Grafana OnCall webhook fires to IncidentResponse's ingress endpoint with a P1 alert payload containing `integration_id`, `route_id`, `team assignment`, and `alert_group_id` (used as `incident_id`).
...
6. **Create Slack war room** — `conversations.create` (private channel, name: `incident-response-p1-{YYYYMMDD}-{short_incident_id}`)
...
**War room name format:** `incident-response-p1-{YYYYMMDD}-{short-id}` (e.g., `incident-response-p1-20250115-abc1`)
...
**Nudge timer:** IncidentResponse starts a 15-minute countdown after war-room creation. If no status update is posted by the IC in the war-room within 15 minutes, IncidentResponse posts an ephemeral reminder to the IC only (not the full channel).
...
**Status update detection:** IncidentResponse watches for any message by the IC user_id in the war-room channel and resets the 15-minute timer on each IC message. (Non-IC messages do not reset the timer.)

**Rate limit:** Maximum 1 nudge per 5-minute window per incident. If IC has not responded within 30 minutes of the last nudge, IncidentResponse escalates by DM-ing the secondary on-call (if defined in Grafana OnCall escalation chain) with a note that the IC has not posted updates.

### 4.3 Status Page Draft → Approve → Publish Flow

**Draft creation trigger:** IC types `/incident-response status draft` OR IncidentResponse auto-proposes a draft when checklist item "Status page draft created" becomes due (after 15 minutes if no draft exists).
...
- IncidentResponse posts the draft to the war-room as a formatted message with two buttons: **✅ Approve & Publish** and **✏️ Edit Draft**
- "Approve & Publish": 
...
**Trigger:** IC runs `/incident-response resolve` OR Grafana OnCall sends `alert_group_resolved` webhook.

**Sequence:**
1. IncidentResponse updates incident status to `RESOLVED` in DynamoDB
2. Posts ephemeral message to IC only:
   > 🎉 Incident resolved. How well did IncidentResponse help you think clearly?
   > [⭐] [⭐⭐] [⭐⭐⭐] [⭐⭐⭐⭐] [⭐⭐⭐⭐⭐]
...
**If IC does not rate within 24h:** IncidentResponse sends one follow-up DM. If still no rating after 48h, incident is marked `RATING_SKIPPED` — not counted in rolling average.

### 4.5 Postmortem Draft (Linear)

**Trigger:** Incident resolved (either via webhook or `/incident-response resolve`).
...
|---------|-------------|-------------|
| `/incident-response status` | Show current incident status summary | Any war-room member |
| `/incident-response status draft` | Trigger Bedrock status page draft | IC only |
| `/incident-response resolve` | Manually trigger incident resolution | IC only |
| `/incident-response checklist` | Re-post the incident checklist | IC only |
| `/incident-response invite @user` | Manually invite an additional responder | IC only |
| `/incident-response silence` | Silence status-update nudges for this incident | IC only |
| `/incident-response help` | Show available commands | Any war-room member |
...
1. Incident detection/alerting — IncidentResponse consumes Grafana OnCall alerts, never generates or modifies them
2. Automatic remediation — IncidentResponse is communication/coordination only; no write access to production systems
...
6. Postmortem root-cause analysis — IC authors the analysis; IncidentResponse provides the populated template
...
| Statuspage.io component selection | IC selects affected components from multi-select at war-room creation or from `/incident-response status draft` prompt |
...
| Resolution trigger contract | Grafana OnCall `alert_group_resolved` webhook OR IC `/incident-response resolve`; postmortem created within 2 min |
