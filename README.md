...
- **platform.yaml** — Platform CR (`agents.stxkxs.io/v1alpha1`) declaring marshal as a tenant of the `protohype` team, with a co-declared BudgetPolicy ($2500/mo soft cap, kill-switch on, alerts at 50/80/100%). `identity.allowedModelFamilies: ["anthropic"]` for Bedrock access on the operator-reconciled IRSA role (used by AgentFleet pods if/when any land); marshal's own app pods assume the landing-zone-owned role directly via `aws.platformRoleArn`.
