...
- Cluster addons (cert-manager, external-secrets, external-dns, the AWS Load Balancer Controller, Grafana Alloy, Loki, Tempo, grafana-operator) → [`eks-gitops`](https://github.com/nanohype/eks-gitops). That catalog serves no `nginx` ingress class, which `webhook-ingress.yaml` requests; see [`chart/README.md`](chart/README.md).
